...
export default function Formulario({ onClose, onSubmit }: FormularioProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Cargar contactos
        const contactsRes = await fetch('http://localhost:3002/newsale/ContactsByUser');
        if (!contactsRes.ok) throw new Error('Error loading contacts');
        setContacts(await contactsRes.json());

        // Cargar productos
        const productsRes = await fetch('http://localhost:3002/newsale/AllProd');
        if (!productsRes.ok) throw new Error('Error loading products');
        setProducts(await productsRes.json());

        // Cargar fases
        const phasesRes = await fetch('http://localhost:3002/newsale/Phases');
